...
Agent Dev 是一个本地开发辅助工具。让你通过按住 `Option` / `Alt` 点选页面上的 DOM，打开 Agent 面板，把目标元素和修改要求一起交给本地 CLI Agent 执行，并在页面里持续查看任务输出。
...
- Vite dev server 自动注入 Agent Dev 客户端，支持 `Option` / `Alt` 点选 DOM 打开面板。
...
npx agent-dev init --bundler webpack
```

只改配置、不安装依赖：

```bash
npx agent-dev init --no-install
```
...
import agentDev from '@agent-dev/vite' // <-- 引入插件
...
`agentDev()` 需要放在 React/Vue/Svelte 等框架插件前面，这样开发态 source 标记会在框架编译 JSX / template 前注入。

启动 dev server 后：

- `Option` / `Alt` + 点击页面元素：打开 Agent 面板并选中目标。
...
CODEX_CLI=codex
CLAUDE_CLI=claude
CODEX_INSPECT_PROXY=http://127.0.0.1:7890
...
```txt
packages/cli       # agent-dev 命令，提供 init
